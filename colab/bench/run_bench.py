...
Env config: IMPL, CFG(tiny|default), ITERS, WARMUP, NODE.
Reads src/impls.py + src/config.py + src/random_state.py from ./src (uploaded).
Prints: first line = CORRECTNESS json, second line = LATENCY json.
"""
...
def main():
    impl = os.environ.get("IMPL", "numpy_opt2")
    ...
    # correctness vs f64 ref (same as evals/eval_correctness.py)
    W64 = RNG(0).weights(cfg, dtype=np.float64)
    from src.impls import IMPLS as M
